  ...
  return async (dispatch) => {
    ...
    try {
      const { data } = await axios.get(
        `http://localhost:1337/product/${productId}/review`
      );
  ...
  return async (dispatch) => {
    try {
      const { data } = await axios.put(
        `http://localhost:1337/product/${productId}/review/${idReview}`,
        review
      );
...
export function removeReview(productId, idReview) {
  return async (dispatch) => {
    try {
      dispatch(loading());
      await axios.delete(`http://localhost:1337/product/${productId}/review/${idReview}`);
