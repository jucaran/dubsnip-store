...
export function addUser(newUser, cart) {
  return async (dispatch) => {
    dispatch(loading());
    try {
      await axios.post("http://localhost:1337/auth/register", newUser);
      dispatch(idle());
    } catch (error) {
      dispatch(failed());
      console.error(error);
...
export function confirmEmail() {
  return async (dispatch) => {
    dispatch(loading());
    try {
      await axios.put("http://localhost:1337/auth/confirmEmail");
      dispatch(idle());
    } catch (err) {
      console.log(err);
      dispatch(failed());
...
export function resetUserPassword(values, id, token) {
  return async (dispatch) => {
    ...
    try {
      ...
      await axios.post(
        `http://localhost:1337/users/${id}/passwordReset`,
        password,
        {
          headers: {
...
export function searchByEmail({ email }) {
  return async (dispatch) => {
    dispatch(loading());
    try {
      await axios.get(`http://localhost:1337/users/email?email=${email}`);
      dispatch(idle());
    } catch (err) {
      console.error(err);
      dispatch(failed());
  ...
  return async (dispatch) => {
    ...
    try {
      const { data } = await axios.put(
        `http://localhost:1337/auth/promote/${id}`,
        {
          isAdmin,
        }
      );
...
export function getUser() {
  return async (dispatch) => {
    dispatch(loading());
    try {
      const { data } = await axios.get(`http://localhost:1337/auth/me`);
...
export function sendEmailContact({name, email, message}) {
  return async (dispatch) => {
    dispatch(loading());
    try {
      const data = await axios.post(`http://localhost:1337/users/emailContact`, {
        name, email, message 
      });
      dispatch(idle());
    } catch (err) {
